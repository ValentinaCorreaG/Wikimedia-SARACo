    ...
    def fetch_metrics(self):
        ...
        try:
            ...
            return {
                ...
                "words_added": self.parse_human_number(
                    campaign.get("word_count_human", 0)
                ),
                "references_added": self.parse_human_number(
                    campaign.get("references_count_human", 0)
                ),
                "article_views": self.parse_human_number(
                    campaign.get("view_sum_human", 0)
                ),
                "articles_edited": self.parse_human_number(
                    campaign.get("article_count_human", 0)
                ),
                "articles_created": self.parse_human_number(
                    campaign.get("new_article_count_human", 0)
                ),
                "commons_uploads": self.parse_human_number(
